...
/**
 * AudioOutput allows the access of system audio output in pure-java manner. It
 * uses the javax.sound library (package).
 */
public class AudioOutput {
	...
	public static void initAudioLine() {
		/*
		 * println("Available Mixers:"); Mixer.Info[] aInfos=AudioSystem.getMixerInfo();
		 * for (int i=0; i < aInfos.length; i++)
		 * print("   "+i+") "+aInfos[i].getName()+"\n"); if (aInfos.length == 0) {
		 * println("WARNING: NO mixers available."); //System.exit(0); return; }
		 */
		...
		try {
			...
			if (DEBUG)
				println("SourceDataLine: " + source_line);
	...
	private void init(AudioFormat format) {
		...
		if (format == null) { // by default use 8000 Hz, ULAW, 8bit, Mono
			float fFrameRate = 8000.0F;
			format = new AudioFormat(AudioFormat.Encoding.ULAW, fFrameRate, 8, 1, 1, fFrameRate, false);
		}

		if (source_line.isOpen()) { // convert the audio stream to the selected format
			try {
	...
	public OutputStream getOuputStream() { // return output_stream;
