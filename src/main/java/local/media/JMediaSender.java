...
public class JMediaSender {
		...
		// try
		{
			if (media == null) {
				if (type.equals("audio"))
					media_locator = getAudio();
				else
					media_locator = getVideo();
			} else
				media_locator = new MediaLocator(media);
		}
			...
			for (int i = 0; i < tracks.length; i++) {
				...
				if (!enabled) {
					if (format.toString().indexOf("Stereo") > 0 || format.toString().indexOf("Mono") > 0) {
						if (type.equals("audio"))
							enabled = track_control.setFormat(new AudioFormat(AudioFormat.GSM_RTP)) != null;
						else
							enabled = false;
					} else {
						if (type.equals("video"))
							enabled = track_control.setFormat(new VideoFormat(VideoFormat.H263_RTP)) != null;
						else
							enabled = false;
					}
	...
	private MediaLocator getMediaDevice(Format format) {
		if (format != null)
			System.out.println("Selected format: " + format.toString());
		else
			System.out.println("Selected format: any");
		...
		for (int i = 0; i < deviceList.size(); i++) {
			CaptureDeviceInfo di = deviceList.get(i);
			System.out.println("device " + i + ":" + di.getName());
		}

		CaptureDeviceInfo di = deviceList.get(deviceList.size() - 1);
