		...
		try {
			...
			while (running) {
				try { // read a block of data from the rtp socket
					...
					if (running)
						output_stream.write(rtp_packet.getPacket(), rtp_packet.getHeaderLength(),
								rtp_packet.getPayloadLength());
					/*
					 * if (running) { byte[] pkt=rtp_packet.getPacket(); int
					 * offset=rtp_packet.getHeaderLength(); int len=rtp_packet.getPayloadLength();
					 * int pos=0; for (int i=0; i<len; i++) { int
					 * linear=G711.ulaw2linear(pkt[offset+i]); //aux[pos++]=(byte)(linear&0xFF);
					 * //aux[pos++]=(byte)((linear&0xFF00)>>8);
					 * aux[pos++]=(byte)G711.linear2ulaw(linear); } output_stream.write(aux,0,pos);
					 * }
					 */
