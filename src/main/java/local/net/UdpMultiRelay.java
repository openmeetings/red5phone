...
public class UdpMultiRelay extends Thread {
	...
	public UdpMultiRelay(int local_port, Vector<SocketAddress> dest_sockets, SocketAddress no_dest_socket) {
		init(local_port, dest_sockets, no_dest_socket);
		start();
	}

	/** Creates a new UDP relay and starts it */
	public UdpMultiRelay(int local_port, Vector<SocketAddress> dest_sockets) {
		init(local_port, dest_sockets, null);
		start();
	}

	/** Inits a new UDP relay and starts it */
	private void init(int local_port, Vector<SocketAddress> dest_sockets, SocketAddress no_dest_socket) {
	...
	public Vector<SocketAddress> getDestSockets() {
		return dest_sockets;
	}
	...
	public void run() {
		try {
			...
			while (!stop) {
				...
				for (SocketAddress dest : dest_sockets) {
					try {
