...
public class UdpRelay extends Thread {
	 ...
	 * Creates a new UDP relay and starts it.
	 * <p>
	 * The UdpRelay will automatically stop after <i>alive_time</i> seconds of idle
	 * time (i.e. without receiving UDP datagrams)
	 */
	 ...
	 * addr/port
	 */
	public void run() { // System.out.println("DEBUG: starting UdpRelay "+toString()+" (it expires after
						// "+alive_to+" sec)");
		try {
			...
			while (!stop) { // non-blocking receiver
				...
				} catch (InterruptedIOException ie) { // if expired, stop relaying
				...
				if (src_port != packet.getPort() || !src_addr.equals(packet.getAddress().getHostAddress().toString())) {
					...
					if (listener != null)
						listener.onUdpRelaySourceChanged(this, src_addr, src_port);
				}
				// relay
