...
{
	private static Logger log = LoggerFactory.getLogger(NotifierDialog.class);
	/** String "active" */
	...
	public void listen() {
		log.debug("inside method listen()");
		if (!statusIs(D_INIT)) {
			log.debug("first subscription already received");
			return;
		}
		// else
	...
	public void notify(Message req) {
		...
		if (subscription_state.equalsIgnoreCase(ACTIVE) && (statusIs(D_SUBSCRIBED) || statusIs(D_PENDING)))
	...
	public void onReceivedMessage(SipProvider provider, Message msg) {
		...
		if (msg.isRequest() && msg.isSubscribe()) {
			if (statusIs(D_WAITING)) { // the first SUBSCRIBE request
				changeStatus(D_SUBSCRIBED);
			...
			log.warn("message is not a SUBSCRIBE: message discarded");
		}
	}
}
