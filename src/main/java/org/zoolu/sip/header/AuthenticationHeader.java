...
public abstract class AuthenticationHeader extends Header {
	...
	public static String[] QUOTED_PARAMETERS = { "auts", "cnonce", "nextnonce", "nonce", "opaque", "realm", "response",
			"rspauth", "uri", "username" };
	...
	/**
	 * Creates a new AuthenticationHeader. specifing the <i>auth_scheme</i> and the
	 * vector of authentication parameters.
	 * <p>
	 ...
	 * qop=null; while ((qop=par.getWord(separators))!=null) aux.addElement(qop); if
	 * (aux.size()==0) return null; String[] qop_options=new String[aux.size()]; for
	 * (int i=0; i<qop_options.length; i++) qop_options[i]=(String)aux.elementAt(i);
	 * return qop_options; }
	 */
	/** Gets the qop-options */
	...
	/*
	 * public void addQopOptionsParam(String[] qop_options) { StringBuffer sb=new
	 * StringBuffer(); for (int i=0; i<qop_options.length; i++) { if (i>0)
	 * sb.append(","); sb.append(qop_options[i]); }
