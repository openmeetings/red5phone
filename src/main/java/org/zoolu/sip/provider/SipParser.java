...
public class SipParser extends Parser {
	...
	/**
	 * Creates a new SipParser based on String <i>s</i> and starting from position
	 * <i>i</i>
	 */
	...
	public static boolean isMark(char c) { // return (c=='-' || c=='_' || c=='.' || c=='!' || c=='~' || c=='*' ||
											// c=='\'' || c=='|');
	 ...
	 * include '\r' and '\n'
	 */
	public static boolean isSeparator(char c) { // return (isSpace(c) || isCRLF(c) || c=='(' || c==')' || c=='<' ||
												// c=='>' || c==',' || c==';' || c=='\\' || c=='"' || c=='/' || c=='['
												// || c==']' || c=='?' || c=='=' || c=='{' || c=='}');
	...
	public int indexOfEOH() {
		...
		while (true) {
			...
			if (!par.hasMore())
				return str.length(); // if no CR/LF found return the end of string
			int end = par.getPos();
	...
	/*
	 * public static SipURL parseSipURL(String s) { SipParser par=new SipParser(s);
	 * return par.parseSipURL(); }
	 */
	/**
	 * Returns the first NameAddress in the string <i>str</i>. If no NameAddress is
	 * found, it returns <b>null</b>. A NameAddress is a string of the form of: <BR>
	 * <BLOCKQUOTE>
	 * 
	 * <PRE>
	...
	/**
	 * Skips the first NameAddress. If no NameAddress is found, it goes to the end
	 * of the string.
	 */
	/*
	 * public SipParser skipNameAddress() { int begin_url=indexOf("<sip:"); if
	 * (begin_url<0) begin_url=indexOf("sip:"); if (begin_url<0) index=str.length();
	 * else { goTo(uri_separators); skipChars(uri_separators); } return this; }
	 */

	/**
	 * Returns the first quoted string in the string <i>str</i>; if no NameAddress
	 * is found, it returns <b>null</b>
	 */
	/*
	 ...
	 * 
	 * @returns the parameter value or null if parameter does not exist or doesn't
	 *          have a value (i.e. in case of flag parameter).
	 */
	...
	/**
	 * Goes to the first comma-separator. Goes to the end of string if no comma is
	 * found.
	 */
	...
	/**
	 * Gets the first SIP message (all bytes until the first end of SIP message), if
	 * a SIP message delimiter is found.
	 * <p>
