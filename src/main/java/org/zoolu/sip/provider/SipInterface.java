 ...
 * <p/>
 * SipInterface provides a simple interface to the multiplexing function
 * provided by the SipProvider layer. <br/>
 * It simply wraps the SipProvider by adding and removing the listener for
 * capturing received SIP messages.
 * <p/>
 * When creating a new SipInterface the following parameters, have to be
 * specified: <b> - <i>sip_provider<i/> is the SipProvider the SipInterface has
 * to be bound to, <b> - <i>id<i/> is the SIP interface identifier the
 * SipInterface has to be bound to, <b> - <i>listener<i/> is the
 * SipInterfaceListener that received messages are passed to.
 * <p/>
 * The SIP interface <i>id<i/> specifies the type of messages the listener is
 * going to receive for. Together with the <i>sip_provider<i/>, it represents
