...
public abstract class BaseMessageFactory {
	 ...
	 */
	public static Message createRequest(String method, SipURL request_uri, NameAddress to, NameAddress from,
			NameAddress contact, String proto, String via_addr, int host_port, boolean rport, String call_id, long cseq,
	 ...
	 * <LI>transport protocol is taken from request-uri (if transport parameter is
	 * present) or the default transport for the SipProvider is used.
	 * </UL>
	 * 
	 * @param sip_provider
	 ...
	 */
	public static Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to,
			NameAddress from, NameAddress contact, String call_id, long cseq, String local_tag, String remote_tag,
		...
		return createRequest(method, request_uri, to, from, contact, proto, via_addr, host_port, rport, call_id, cseq,
				local_tag, remote_tag, branch, body);
	}
	 ...
	 */
	public static Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to,
			NameAddress from, NameAddress contact, String body) { // SipURL request_uri=to.getAddress();
		...
		return createRequest(sip_provider, method, request_uri, to, from, contact, call_id, cseq, local_tag, null, null,
				body);
	}
	 ...
	 */
	public static Message createRequest(SipProvider sip_provider, String method, NameAddress to, NameAddress from,
			String body) {
		String contact_user = from.getAddress().getUserName();
		NameAddress contact = new NameAddress(
	...
	public static Message createRequest(Dialog dialog, String method, String body) {
		...
		// String branch=SipStack.pickBranch();
		Message req = createRequest(method, request_uri, to, from, contact, proto, via_addr, host_port, rport, call_id,
				cseq, local_tag, remote_tag, null, body);
	...
	/**
	 * Creates a SIP ACK request message within a dialog, with a same branch
	 * via-parameter.
	 * 
	 * @param dialog
	 *            the Dialog used to compose the various Message headers
	 ...
	 */
	public static Message createInviteRequest(SipProvider sip_provider, SipURL request_uri, NameAddress to,
		...
		if (contact == null)
			contact = from;
		return createRequest(sip_provider, SipMethods.INVITE, request_uri, to, from, contact, call_id, cseq, local_tag,
				null, null, body);
	}
	...
	public static Message createNon2xxAckRequest(SipProvider sip_provider, Message method, Message resp) {
		...
		Message ack = createRequest(SipMethods.ACK, request_uri, to.getNameAddress(), from.getNameAddress(), contact,
				proto, via_addr, host_port, rport, method.getCallIdHeader().getCallId(),
				method.getCSeqHeader().getSequenceNumber(), from.getParameter("tag"), to.getParameter("tag"), branch,
				null);
	 ...
	 * (SipStack.contact_url!=null) contact=new NameAddress(SipStack.contact_url);
	 * return createRequest(SipMethods.ACK,request_uri,to.getNameAddress(),from.
	 * getNameAddress(),contact,resp.getCallIdHeader().getCallId(),resp.
	 * getCSeqHeader().getSequenceNumber(),from.getParameter("tag"),to.getParameter(
	 * "tag"),branch,body); }
	 */

	/** Creates an ACK request for a 2xx-response within a dialog */
	/*
	 * public static Message create2xxAckRequest(Dialog dialog, NameAddress contact,
	 * String body) { return createRequest(SipMethods.ACK,dialog,contact,body); }
	 */
	...
	public static Message createCancelRequest(Message method) {
		...
		String branch = method.getViaHeader().getBranch();
		return createRequest(SipMethods.CANCEL, request_uri, to.getNameAddress(), from.getNameAddress(), contact, proto,
				host_addr, host_port, rport, method.getCallIdHeader().getCallId(),
				method.getCSeqHeader().getSequenceNumber(), from.getParameter("tag"), to.getParameter("tag"), branch,
				"");
	}
	...
	public static Message createRegisterRequest(SipProvider sip_provider, NameAddress to, NameAddress from,
		...
		String local_tag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		Message req = createRequest(SipMethods.REGISTER, registrar, to, from, contact, proto, via_addr, host_port,
				rport, call_id, cseq, local_tag, null, null, null);
	 ...
	 *            the message body
	 */
	public static Message createResponse(Message req, int code, String reason, String local_tag, NameAddress contact,
	...
	/**
	 * Creates a SIP response message. For 2xx responses generates the local tag by
	 * means of the SipStack.pickTag(req) method.
	 * 
	 * @see #createResponse(Message,int,String,NameAddress,String,String body)
	 */
	public static Message createResponse(Message req, int code, String reason, NameAddress contact) { // String
